...
# see https://github.com/cachix/install-nix-action/blob/master/install-nix.sh
...
# curl_retries=5
# while ! curl -sS -o "$workdir/install" -v --fail -L "${INPUT_INSTALL_URL:-https://releases.nixos.org/nix/nix-2.20.5/install}"; do
#   sleep 1
#   ((curl_retries--))
...
sh <(curl -L https://nixos.org/nix/install) # "${installer_options[@]}"
